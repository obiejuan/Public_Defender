 ...
 * Checks
 * 
 * @param {object} req Express_js request object.
 * @param {object} res Express_js response object.
 * @param {function} next Object (function) used to go to the next middleware
 * @returns {object} Function `next` used to go to the next middleware function.
 */
function check_login(req, res, next){
	var user_id_token = req.headers['auth-key'];
	if (user_id_token) { //if they have token in headers
		...
		client.verifyIdToken(user_id_token, CLIENT_ID, (e, login) => {
			if (e) { // was there an error?
				...
				*	
				* 		SQL: 'select_user' --> @file check_usr_exists.sql
				*		Var: q_usr_create
				***********************************************************/
